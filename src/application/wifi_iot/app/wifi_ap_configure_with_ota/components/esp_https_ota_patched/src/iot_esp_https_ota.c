...
esp_err_t iot_esp_https_ota_begin(const esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle, esp_https_ota_head_t *head)
{
    esp_https_ota_dispatch_event(ESP_HTTPS_OTA_START, NULL, 0);
    ...
    esp_http_client_set_header(https_ota_handle->http_client,"Content-Type", head->Content_Type);
    esp_http_client_set_header(https_ota_handle->http_client, "Authorization", head->Authorization);
...
esp_err_t iot_esp_https_ota(const esp_https_ota_config_t *ota_config, esp_https_ota_head_t *head)
{
    if (ota_config == NULL || ota_config->http_config == NULL) {
        ESP_LOGE(TAG, "esp_https_ota: Invalid argument");
        return ESP_ERR_INVALID_ARG;
    ...
    esp_https_ota_handle_t https_ota_handle = NULL;
    esp_err_t err = iot_esp_https_ota_begin(ota_config, &https_ota_handle, head);
    if (https_ota_handle == NULL) {
        return ESP_FAIL;
    }
