...
int oc_mqtt_send_version_report(esp_mqtt_client_handle_t *mqtt_handle, const char *device_id, char *version) {
    char topic[256];
    int ret = 0;
    snprintf(topic, sizeof(topic), IOT_OC_MQTT_PROFILE_OTA_TOPICFMT_UP, device_id);
    ...
    cJSON_AddStringToObject(service, "event_type", "version_report");

    cJSON *paras = cJSON_CreateObject();
    cJSON_AddStringToObject(paras, "fw_version", version);
    ...
    if (payload != NULL) {
        ESP_LOGI("MQTT", "Publishing version report:\n %s", payload);
        ret = esp_mqtt_client_publish(*mqtt_handle, topic, payload, strlen(payload), 1, 0);
        if (ret < 0) {
            printf("Failed to publish version report\n");
            return -1;
        }
        free(payload);
        return 0;
